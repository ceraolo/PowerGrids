...
equation
  connect(NTLV.terminal, TGEN.terminalA) annotation (
    Line(points = {{-42, -1.42109e-15}, {-30, -1.42109e-15}}));
  connect(TGEN.terminalB, NTHV.terminal) annotation (
    Line(points = {{-10, 0}, {4, 0}}));
  connect(NTHV.terminal, GRID.terminal) annotation (
    Line(points = {{4, -1.42109e-15}, {10, -1.42109e-15}, {10, 10}, {30, 10}}));
  connect(NTHV.terminal, GRIDL.terminal) annotation (
    Line(points = {{4, -1.42109e-15}, {10, -1.42109e-15}, {10, -12}, {24, -12}}));
  connect(ctrlGEN.terminal, NTLV.terminal) annotation (
    Line(points = {{-64, 0}, {-42, 0}}));
  annotation (
    Icon(coordinateSystem(grid = {0.1, 0.1})),
    Diagram(coordinateSystem(extent = {{-80, 60}, {40, -20}}, grid = {0.5, 0.5})),
